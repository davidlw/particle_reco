...
class LambdaCData : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
    ...
    private:
        ...
        //for internal use
        vector<Float_t> eventInfo;  //vector for event info: has elements: ntrkoffline, vtxX, vtxY, vtxZ in that order
        TNtuple* lambdaTree;
        TNtuple* piTree;
...
LambdaCData::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    //reset event/daughter info collections
    eventInfo.erase(eventInfo.begin(), eventInfo.end());
    eventInfo.reserve(4);

    lambdaTree = new TNtuple("lambdaTree", "lambdaTree", "px:py:pz:pt:eta:m:agl:dlsig:is_anti");
    piTree = new TNtuple("piTree", "piTree", "px:py:pz:p:pt:pterr:eta:q:dedx:dcaz:dcaxy:nhits");
    ...
    //store event info
    eventInfo.push_back(ntrk);
    eventInfo.push_back(bestvx);
    eventInfo.push_back(bestvy);
    eventInfo.push_back(bestvz);
    ...
    for(unsigned i = 0; i < v0candidates_la->size(); i++)
    {
        ...
        if(la_proton->charge() == 1)
            ...
            eventInfo[0] = -999.9;  //safeguard: if proton charge is odd, then ntrkoffline will be set to -999.9
    ...
    //analysis
    recoData(eventInfo, lambdaTree, piTree, lcrecoTree);  //reconstructs the LambdaC and fill lcrecoTree
...
LambdaCData::beginJob()
{
    ...
    string generalEventInfo = "ntrkoffline";
    string LambdaCInfo = "mass:pt:eta";
    string dau1Info = "mass_dau1:pt_dau1:eta_dau1:3DPointingAngle_dau1:3DDecayLengthSig_dau1";
    string dau2Info = "pt_dau2:ptErr_dau2:p_dau2:eta_dau2:zDCASig_dau2:xyDCASig_dau2:dEdxHarmonic_dau2:nhits_dau2";

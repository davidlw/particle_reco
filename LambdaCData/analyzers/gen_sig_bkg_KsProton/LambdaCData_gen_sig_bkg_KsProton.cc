...
class LambdaCData : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
    ...
    private:
        ...
        TNtuple* matchedKsTree;
        TNtuple* matchedProtonTree;
...
LambdaCData::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    for(unsigned i = 0; i < genpars->size(); i++)
    {
        ...
        //(There shouldn't be any LambdaPi daughters, but just in case...)
    ...
    for(int i = 0; i < nGenDau; i+=2)
    {
        ...
        matchedKsTree = new TNtuple("matchedKsTree", "matchedKsTree", "px:py:pz:pt:eta:m:agl:dlsig:stat");
        matchedProtonTree = new TNtuple("matchedProtonTree", "matchedProtonTree", "px:py:pz:p:pt:pterr:eta:q:dedx:dcaz:dcaxy:nhits");

        //go through each Ks
        for(unsigned j = 0; j < v0candidates_ks->size(); j++)
        {
            //pick out the Ks that match with the current true Ks
            const reco::VertexCompositeCandidate & ks = (*v0candidates_ks)[j];

            //standard cuts
            double eta_ks = ks.eta();

            if(fabs(eta_ks) > 2.4) continue;
            //end standard cuts

            //compare the current Lambda with the true one
            if(!compareTrk(ks, Dvect1)) continue;
            ...
            //fill the dau1 info tree
             matchedKsTree->Fill(px_ks, py_ks, pz_ks, ks.pt(), \
                eta_ks, ks.mass(), angle, dlos, Dvect1[4]);
        }
        ...
        for(unsigned j = 0; j < tracks->size(); j++)
        {
            ...
            matchedProtonTree->Fill(trk.px(), trk.py(), trk.pz(), trk.p(), trk.pt(), trk.ptError(), \
        ...
        recoData(matchedEventInfo, matchedKsTree, matchedProtonTree, lcMatchTree);


        //clear the matched daughter trees for the next true daughter pair
        delete matchedKsTree;
        delete matchedProtonTree;
    }
    ...
    for(unsigned i = 0; i < v0candidates_ks->size(); i++)
    {
        ...
        bkgKsTree = new TNtuple("bkgKsTree", "bkgKsTree", "px:py:pz:pt:eta:m:agl:dlsig:stat");

        bkgKsTree->Fill(px_ks, py_ks, pz_ks, ks.pt(), \
            eta_ks, ks.mass(), angle, dlos, -999.9);  //status_matched_dau1 is given the garbage value -999.9
        ...
        for(unsigned j = 0; j < tracks->size(); j++)
        {
            ...
            for(int k = 0; k < nGenDau; k+=2)
            {
                ...
                if( compareTrk(ks, Dvect1) && compareTrk(trk, Dvect2) )
                    pair_matches = true;
            }
            ...
            bkgProtonTree = new TNtuple("bkgProtonTree", "bkgProtonTree", "px:py:pz:p:pt:pterr:eta:q:dedx:dcaz:dcaxy:nhits");

            bkgProtonTree->Fill(trk.px(), trk.py(), trk.pz(), trk.p(), trk.pt(), trk.ptError(), \
                eta_trk, trk.charge(), H2dedx, dcaz_trk, dcaxy_trk, trk.numberOfValidHits());

            //do reconstruction on the background and fill lcBkgTree with 1 leaf
            recoData(bkgEventInfo, bkgKsTree, bkgProtonTree, lcBkgTree);

            // Clear bkgProtonTree every loop so it never has more than 1 entry
            delete bkgProtonTree;
        }

        // Clear bkgKsTree every loop so it never has more than 1 entry
        delete bkgKsTree;
    }
}
...
LambdaCData::beginJob()
{
    ...
    string dau1Info = "mass_dau1:pt_dau1:eta_dau1:y_cm_dau1:3DPointingAngle_dau1:3DDecayLengthSig_dau1:status_dau1_matched";

...
LambdaCData::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    {        
        ...
        // Apply dE/dx cuts
        double proton_mass = 0.9382720813;
        if( !( (H2dedx_dau2 >= 2.6 * pow(proton_mass / (p_dau2 + 0.1), 2) + 2.2 || p_dau2 >= 2.2) && \
            (H2dedx_dau2 < 3.5 * pow(proton_mass / (p_dau2 - 0.1), 2) + 4.2 || p_dau2 >= 2.5) ) )
        {
            continue;
        }

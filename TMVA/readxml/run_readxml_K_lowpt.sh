...
ssh bonner${SERVER_LIST[$i]} "
    ...
    for j in ${INDICES[$i]}; do
        nohup unbuffer root -l -b -q \`echo 'readxml.cc+(\"${CONFIG_FILE}\",\"/home/jg61/particle_reco/InputChain/config/split_K/dataTrees_K_'\${j}'.xml\",\"${OUT_TAG}_'\${j}'\")'\` &> ${LOG_PATH}/readxml${OUT_TAG}_\${j}.log &
    done;"

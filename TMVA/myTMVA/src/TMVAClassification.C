...
void TMVAClassification(const string&, TString myMethodList = "");

int main(int argc, char* argv[])
{
    if(argc <= 1)
    {
        cout << "ERROR: Please pass in the name of the configuration file. Exiting.\n";
        exit(-1);
    }

    string config_file_name = argv[1];
    if(argc > 2)
        TMVAClassification(config_file_name, argv[2]);
    else
...
void TMVAClassification( const string& config_file_name, TString myMethodList = "" )
{
    ...
    if (Use["CutsSA"])
        factory->BookMethod( TMVA::Types::kCuts, "CutsSA",
                                    "!H:!V:FitMethod=SA:EffSel:MaxCalls=150000:KernelTemp=IncAdaptive:InitialTemp=1e+6:MinTemp=1e-6:Eps=1e-10:UseDefaultScale" + varProps );

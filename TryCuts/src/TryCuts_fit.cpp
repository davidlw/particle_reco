...
void TryCuts_fit(const string& config_file_name, const string& output_tag)
{
    ...
    // Open the histos output file generated by TryCuts
    ...
    for(int icut = 1; icut <= cfg.n_histos(); icut++)
    {
        ...
        c->cd();
        h = (TH1D*)histoFile.Get(("cut_" + to_string(icut)).c_str());
        ...
        c->Print((outPath + "_" + to_string(icut) + output_tag + imageType).c_str());
